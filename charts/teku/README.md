...
| devnet | object | `{"baseUrl":"https://raw.githubusercontent.com/ethpandaops/fusaka-devnets/master/network-configs","enabled":false,"initContainer":[{"command":["sh","-ac","mkdir -p /data/testnet_spec;\nif [ ! -f /data/testnet_spec/genesis.ssz ]; then\n  echo \"Downloading devnet configuration files...\";\n\n  # Download deposit contract block (required by Teku for --eth1-deposit-contract-address)\n  echo \"Downloading deposit_contract_block.txt from {{ tpl .Values.devnet.urls.depositContractBlock . }}\"\n  wget -O /data/testnet_spec/deposit_contract_block.txt \"{{ tpl .Values.devnet.urls.depositContractBlock . }}\"\n\n  # Download deposit contract address\n  echo \"Downloading deposit_contract.txt from {{ tpl .Values.devnet.urls.depositContractAddress . }}\"\n  wget -O /data/testnet_spec/deposit_contract.txt \"{{ tpl .Values.devnet.urls.depositContractAddress . }}\"\n\n  # Download config.yaml (required by Teku --network parameter)\n  echo \"Downloading config.yaml from {{ tpl .Values.devnet.urls.configYaml . }}\"\n  wget -O /data/testnet_spec/config.yaml \"{{ tpl .Values.devnet.urls.configYaml . }}\"\n\n  # Download genesis.ssz (required by Teku --initial-state parameter)\n  echo \"Downloading genesis.ssz from {{ tpl .Values.devnet.urls.genesisSsz . }}\"\n  wget -O /data/testnet_spec/genesis.ssz \"{{ tpl .Values.devnet.urls.genesisSsz . }}\"\n\n  # Download bootstrap nodes (both formats for compatibility)\n  echo \"Downloading bootstrap_nodes.yaml from {{ tpl .Values.devnet.urls.bootstrapNodesYaml . }}\"\n  wget -O /data/testnet_spec/bootstrap_nodes.yaml \"{{ tpl .Values.devnet.urls.bootstrapNodesYaml . }}\"\n  echo \"Downloading bootstrap_nodes.txt from {{ tpl .Values.devnet.urls.bootstrapNodesTxt . }}\"\n  wget -O /data/testnet_spec/bootstrap_nodes.txt \"{{ tpl .Values.devnet.urls.bootstrapNodesTxt . }}\"\n\n  echo \"Devnet configuration download complete.\";\nelse\n  echo \"Genesis file already exists, skipping download.\";\nfi\n"],"image":"alpine:latest","imagePullPolicy":"IfNotPresent","name":"download-devnet-config","securityContext":{"runAsNonRoot":false,"runAsUser":0},"volumeMounts":[{"mountPath":"/data","name":"storage"}]}],"name":"devnet-3","urls":{"bootstrapNodesTxt":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/bootstrap_nodes.txt","bootstrapNodesYaml":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/bootstrap_nodes.yaml","configYaml":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/config.yaml","depositContractAddress":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/deposit_contract.txt","depositContractBlock":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/deposit_contract_block.txt","genesisSsz":"{{ .Values.devnet.baseUrl }}/{{ .Values.devnet.name }}/metadata/genesis.ssz"}}` | Devnet configuration |
